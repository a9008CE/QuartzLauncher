...
public partial class ShaderPacksPage : Page
{
    private readonly ContentService _content = new();
    private string _instanceRoot = "";

    public ShaderPacksPage() : this(null)
    {
    }

    public ShaderPacksPage(string? instanceId)
    {
        InitializeComponent();
    ...
    private void RefreshPacks()
    {
        ...
        var packs = _content.ShaderPacks(_instanceRoot);
        HintText.Text = $"{packs.Count} 个光影包";
    ...
    private void Pack_DoubleClick(object sender, System.Windows.Input.MouseButtonEventArgs e)
    {
        ...
        var result = QuartzLauncher.Services.AnimatedMessageBox.Show($"是否{action}光影包 \"{name}\"?", "确认",
            MessageBoxButton.YesNo, MessageBoxImage.Question);
        if (result == MessageBoxResult.Yes)
        {
            try
            {
                _content.ToggleShaderPack(path);
                RefreshPacks();
            }
            catch (Exception ex)
            {
    ...
    private void Import_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            Filter = "ZIP/JAR archives (*.zip;*.jar)|*.zip;*.jar|All files (*.*)|*.*",
            Multiselect = true
        };
        if (dialog.ShowDialog() == true)
        {
            try
            {
                foreach (var file in dialog.FileNames)
                    _content.ImportShaderPack(_instanceRoot, file);
                RefreshPacks();
            }
            catch (Exception ex)
            {
